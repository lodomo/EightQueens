...
def help(usage_only=False):
    ...
    print("-F\tFamily Tree. Default:Disabled")
...
def visualize_queens(queen, soln=False):
    ...
    color_depth = (255 / MAX_FITNESS) * queen.fitness

    parent_color = [0, 0, color_depth]
    single_color = [color_depth, 0, 0]
    soln_color = [0, color_depth, 0]
    queen_color = None
...
def visualize_generations():
    ...
    pad = 1
    img = np.zeros(
        (len(DATA["FAMILY_TREE"]) * (ROWS + pad), SET["POP_SIZE"] * (COLUMNS + pad), 3),
        dtype=np.uint8,
    )
    ...
    for i in range(len(DATA["FAMILY_TREE"])):
        for j in range(SET["POP_SIZE"]):
            ...
            img[
                i * (ROWS + pad) : i * (ROWS + pad) + ROWS,
                j * (COLUMNS + pad) : j * (COLUMNS + pad) + COLUMNS,
            ] = vis
